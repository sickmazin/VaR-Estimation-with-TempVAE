...
# ==========================================
CONFIG = {
    'seq_length': 29,         # Finestra temporale M
    ...
    'free_bits': 2.0,        # Soglia Free Bits (0.0 per disabilitare)
    'gradient_clip': 5,
...
def train_tempvae():
    ...
    for epoch in range(total_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # 2. Free Bits (Hinge Loss)
            # Non penalizzare la KL se è sotto la soglia. Protegge le units dall'essere azzerate.
            # Se CONFIG['free_bits'] è 0.0, questo equivale a una KL standard.
            kl_effective = torch.max(kl_loss, torch.tensor(CONFIG['free_bits']).to(CONFIG['device']))
...
if __name__ == "__main__":
    # Store the original plot directory
    original_plot_dir = CONFIG['plot_dir']

    for i in range(1,100):
        # Creazione sottocartella basata su timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        run_dir = os.path.join(original_plot_dir, f"run_CPU2_{timestamp}")
        ...
        print(f"Risolto il problema di nidificazione delle directory.")
        print(f"Attualmente la directory per i plot è: {CONFIG['plot_dir']}")

        print(f">>> Risultati di questa esecuzione salvati in: {run_dir}")
        print("\n" + "="*50)
